...
#get matching metadata file
p3 <- synapser::synGet('syn26231239')
...
cells <- which(metadata$sex == 'female')
...
# MonRun2 <- reduceDimension(MonRun, max_components = 2, reduction_method = 'tSNE', verbose=T)
# MonRun2 <- clusterCells(MonRun2)
# plot_cell_clusters(MonRun2, color="Diagnosis")
# plot_cell_clusters(MonRun2, color="ros_ids")
# plot_cell_clusters(MonRun2, color="Subcluster")
...
#set root state to 5
MonRun <- orderCells(MonRun, root_state = 5)
...
g <- g + ggplot2::theme(axis.text=element_text(size=15), axis.title=element_text(size=15,face="bold"),
                        legend.text=element_text(size=15)) 
g <- g + ggplot2::scale_fill_viridis_d()
g <- g + ggplot2::labs(fill="CERAD Score",y="Pseudotime",x="ceradsc")
g
...
#expression matrix:
p <- synapser::synGet('syn26243358')
counts <- readRDS(p$path)

#get matching metadata file
p3 <- synapser::synGet('syn26243359')
metadata <- readRDS(p3$path)
#p3 <- synapser::synGet('syn25871851')
#metadata <- read.csv(p3$path)
...
#expression matrix:
p <- synapser::synGet('syn26243360')
counts <- readRDS(p$path)

#get matching metadata file
p3 <- synapser::synGet('syn26243361')
metadata <- readRDS(p3$path)
#p3 <- synapser::synGet('syn25871851')
#metadata <- read.csv(p3$path)
...
#set root to state 11 
MonRun <- orderCells(MonRun, root_state = 11)
plot_cell_trajectory(MonRun,color_by = "Pseudotime")
plot_cell_trajectory(MonRun,color_by = "State")
...
#expression matrix:
p <- synapser::synGet('syn26243362')
counts <- readRDS(p$path)

#get matching metadata file
p3 <- synapser::synGet('syn26243363')
metadata <- readRDS(p3$path)
#p3 <- synapser::synGet('syn25871851')
#metadata <- read.csv(p3$path)
